...
typedef struct tagFrameInfo{
    unsigned long    SyncId; /* 00000000000000000000000000010b */
    unsigned long    Magic;
    unsigned short    FrameType; /* I frames , P frames or BBP frames Audio frames or dsp status etc */
    unsigned long    Length; /*lenth include this header */
    unsigned long    FrameNumber; /* serial number of this frame */
    unsigned char    Breakable; /* indicate if stream breakable, you could restart new file(with PktSysHeader) if true */
    /*unsigned long   Ack;*/
    unsigned long    PTS; /* system clock when this frames is processed */
